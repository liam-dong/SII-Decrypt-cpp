...
{
    if (!Mem) return SIIDEC_RESULT_GENERIC_ERROR;
    try {
...
{
    if (!FileName) return SIIDEC_RESULT_GENERIC_ERROR;
    try {
...
{
    if (!Input || !OutSize) return SIIDEC_RESULT_GENERIC_ERROR;
    try {
        SIIDecryptor dec;
...
{
    if (!InputFile || !OutputFile) return SIIDEC_RESULT_GENERIC_ERROR;
    try {
        SIIDecryptor dec;
        dec.AcceleratedAES = true;
        dec.ReraiseExceptions = false;
    ...
    try {
        ...
        if (Output && Helper && *Helper) {
            ...
            *Helper = nullptr;
            ...
            if (!hbuf) return SIIDEC_RESULT_GENERIC_ERROR;
...
SII_API Int32 SII_CALL DecodeFile(const char* InputFile, const char* OutputFile)
{
    ...
    try {
        ...
        SIIResult r = dec.GetFileFormat(InputFile);
        if (r != rFormat3nK) {
            if (r == rFormatBinary || r == rFormatEncrypted || r == rFormatPlainText)
                return ToInt32(r);
            return ToInt32(r);
        }
...
SII_API Int32 SII_CALL DecryptAndDecodeFile(const char* InputFile, const char* OutputFile)
{
    ...
    try {
        ...
        SIIResult r = dec.GetFileFormat(InputFile);
        if (r == rFormatEncrypted || r == rFormat3nK) {
...
{
    if (!Decryptor || !Mem) return SIIDEC_RESULT_GENERIC_ERROR;
    try {
        DecryptorObj* obj = (DecryptorObj*)Decryptor;
...
{
    if (!Decryptor || !FileName) return SIIDEC_RESULT_GENERIC_ERROR;
    try {
        DecryptorObj* obj = (DecryptorObj*)Decryptor;
...
SII_API Int32 SII_CALL Decryptor_DecryptMemory(
    ...
    void*       Output, TMemSize* OutSize)
{
    if (!Decryptor || !Input || !OutSize) return SIIDEC_RESULT_GENERIC_ERROR;
    try {
        DecryptorObj* obj = (DecryptorObj*)Decryptor;
...
{
    if (!Decryptor || !InputFile || !OutputFile) return SIIDEC_RESULT_GENERIC_ERROR;
    try {
        DecryptorObj* obj = (DecryptorObj*)Decryptor;
        SIIResult r = obj->Decryptor->GetFileFormat(InputFile);
        if (r != rFormatEncrypted) return ToInt32(r);
    ...
    try {
        ...
        if (!Output) {
            if (!obj->HelperBuffer) {
                ...
                if (!obj->HelperBuffer) return SIIDEC_RESULT_GENERIC_ERROR;
            }
